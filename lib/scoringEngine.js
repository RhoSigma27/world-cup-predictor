...
export function scoreParticipant(predictions, fixtures, extrasPred, masterExtras) {
  ...
  // TEMP DEBUG — remove after diagnosis
     console.log('=== KO RESULTS KEYS ===')
    for (const [round, teams] of Object.entries(userKOResults)) {
        console.log(round, '->', Object.keys(teams).join(', '))
    }
    console.log('=== SCORED KO FIXTURES ===')
    for (const f of fixtures.filter(f => f.round !== 'group' && f.home_score != null)) {
        console.log(`M${f.match_number} ${f.round} | home:"${f.home_team}" away:"${f.away_team}"`)
    }
