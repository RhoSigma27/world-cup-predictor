...
export function isKOReopened(league) {
  const reopen = league?.ko_reopen_until
  if (!reopen) return false
  return new Date() < new Date(reopen)
}

export function getKOReopenUntil(league) {
  const reopen = league?.ko_reopen_until
  if (!reopen) return null
  return new Date() < new Date(reopen) ? new Date(reopen) : null
}