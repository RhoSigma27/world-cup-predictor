...
function deriveKOTeams(tables, fixtures) {
  ...
  // Collect all unique 3rd place slot codes from R32 fixtures
  const r32Fixtures = fixtures.filter(f => f.round === 'R32')
  const thirdSlotCodes = []
  for (const f of r32Fixtures) {
    for (const slot of [f.slot1, f.slot2]) {
      if (slot && /^3[A-L]+$/.test(slot) && !thirdSlotCodes.includes(slot)) {
        thirdSlotCodes.push(slot)
      }
    }
  }

  // Build eligible groups for each slot
  const slots = thirdSlotCodes.map(code => ({
    code,
    eligibleGroups: code.replace(/^3/, '').split('')
  }))

  // Assign best 3rd place teams to slots
  // Each team assigned to exactly one slot, each slot gets exactly one team
  // Use a recursive backtracking assignment for correctness
  const assignedTeams = {}  // slot code -> team object

  function assign(slotIndex) {
    if (slotIndex === slots.length) return true
    const slot = slots[slotIndex]
    for (const team of best3rd) {
      const alreadyAssigned = Object.values(assignedTeams).some(t => t.team === team.team)
      if (!alreadyAssigned && slot.eligibleGroups.includes(team.group)) {
        assignedTeams[slot.code] = team
        if (assign(slotIndex + 1)) return true
        delete assignedTeams[slot.code]
      }
    }
    return false
  }
  ...
  // Add resolved teams to slotMap
  for (const [code, team] of Object.entries(assignedTeams)) {
    slotMap[code] = team.team
  }
...
export default function PredictionsClient({
  ...
  const tables = calcGroupTables(predictions, fixtures)
  const slotMap = allGroupsDone ? deriveKOTeams(tables, fixtures) : {}

  const resolveTeam = (nameOrSlot) => {
    if (!nameOrSlot) return '?'
    if (/^[12][A-L]$/.test(nameOrSlot)) return slotMap[nameOrSlot] || nameOrSlot
    if (/^3[A-L]+$/.test(nameOrSlot)) return slotMap[nameOrSlot] || nameOrSlot
    return nameOrSlot
  }
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {filteredFixtures.map(f => {
                  ...
                  return (
                    <tr key={f.id} className={`border-b border-gray-800/50 transition-colors ${bothFilled ? '' : 'hover:bg-gray-800/50'}`}>
            ...
            <>
              <div className="mt-6">
                <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-4 mb-4">
                  ...
                  <p className="text-gray-500 text-xs mt-1">Predict the score for each knockout match below.</p>
                </div>
              </div>
