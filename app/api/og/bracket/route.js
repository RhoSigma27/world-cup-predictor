...
// Test 9: edge runtime, data loaded, minimal render - no conditionals, no flags, no computed styles
  ...
  const buildRound = (code) =>
    (fixtures || []).filter(f => f.round === code).sort((a, b) => a.match_number - b.match_number)
      .map(f => {
        ...
        return { t1: String(t1 || 'TBD'), t2: String(t2 || 'TBD'), w: pickWinner(pred, t1, t2) }
      })

  const r16 = buildRound('R16')
  const qf  = buildRound('QF')
  const sf  = buildRound('SF')
  const fin = buildRound('FINAL')[0] ?? { t1: 'TBD', t2: 'TBD', w: null }
  const champ = String(fin.w || '?')

  // Absolute minimum render — no conditionals, no flags, no computed styles
    ...
    <div style={{ width: '1200px', height: '630px', background: '#060e1f', display: 'flex', flexDirection: 'column', padding: '36px', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'row', gap: '16px', flex: 1 }}>
        <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-around', width: '180px' }}>
          {r16.map((m, i) => (
            <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
              <div style={{ background: '#0b1525', borderLeft: '2px solid #1a3050', height: '18px', paddingLeft: '6px', paddingRight: '6px', display: 'flex', alignItems: 'center' }}>
                <span style={{ color: '#607a95', fontSize: '10px', whiteSpace: 'nowrap' }}>{m.t1}</span>
              </div>
              <div style={{ background: '#0b1525', borderLeft: '2px solid #1a3050', height: '18px', paddingLeft: '6px', paddingRight: '6px', display: 'flex', alignItems: 'center' }}>
                <span style={{ color: '#607a95', fontSize: '10px', whiteSpace: 'nowrap' }}>{m.t2}</span>
              </div>
            </div>
          ))}
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-around', width: '160px' }}>
          {qf.map((m, i) => (
            <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
              <div style={{ background: '#0b1525', borderLeft: '2px solid #1a3050', height: '26px', paddingLeft: '6px', display: 'flex', alignItems: 'center' }}>
                <span style={{ color: '#607a95', fontSize: '11px', whiteSpace: 'nowrap' }}>{m.t1}</span>
              </div>
              <div style={{ background: '#0b1525', borderLeft: '2px solid #1a3050', height: '26px', paddingLeft: '6px', display: 'flex', alignItems: 'center' }}>
                <span style={{ color: '#607a95', fontSize: '11px', whiteSpace: 'nowrap' }}>{m.t2}</span>
              </div>
            </div>
          ))}
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-around', width: '140px' }}>
          {sf.map((m, i) => (
            <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
              <div style={{ background: '#0b1525', borderLeft: '2px solid #1a3050', height: '35px', paddingLeft: '8px', display: 'flex', alignItems: 'center' }}>
                <span style={{ color: '#607a95', fontSize: '13px', whiteSpace: 'nowrap' }}>{m.t1}</span>
              </div>
              <div style={{ background: '#0b1525', borderLeft: '2px solid #1a3050', height: '35px', paddingLeft: '8px', display: 'flex', alignItems: 'center' }}>
                <span style={{ color: '#607a95', fontSize: '13px', whiteSpace: 'nowrap' }}>{m.t2}</span>
              </div>
            </div>
          ))}
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', width: '120px', gap: '4px' }}>
          ...
          <div style={{ background: '#0e1e35', borderLeft: '2px solid #2a5080', height: '47px', paddingLeft: '9px', display: 'flex', alignItems: 'center' }}>
            <span style={{ color: '#88aed0', fontSize: '14px', whiteSpace: 'nowrap' }}>{fin.t2}</span>
          </div>
        </div>
